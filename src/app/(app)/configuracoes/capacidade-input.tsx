...
import { setCapacidade } from './actions'

export default function CapacidadeInput({ inicial }: { inicial: number | null }) {
  ...
  async function salvar(e: React.FormEvent) {
    ...
    const res = await setCapacidade(num)
    setOcupado(false)
    if (!res.ok) {
      setErro(res.erro)
      return
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-slate-500">
        Máximo de crianças ao mesmo tempo. Vazio = sem limite.
      </p>
